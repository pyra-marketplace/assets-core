...
contract DataUnionTest is BaseTest {
    ...
    function test_Publish_WhenCollectAction() public {
        bytes memory data = abi.encode(testResourceId, testFolderId);
        ...
        IDataUnion.UnionAsset memory unionAsset = dataUnion.getUnionAsset(assetId);
    ...
    function test_Publish_WhenSubscribeAction() public {
        ...
        actionInitDatas[0] = abi.encode(address(monthlySubscribeModule), abi.encode(address(erc20Mock), amount));
        IDataMonetizer.PublishParams memory publishParams =
            IDataMonetizer.PublishParams({data: data, actions: actions, actionInitDatas: actionInitDatas});
    ...
    function test_Act_WhenSubscribeAction() public {
        ...
        assertFalse(subscribeAction.isAccessible(assetId, actor, block.timestamp));
        ...
        bytes[] memory actionProcessDatas = new bytes[](1);
        actions[0] = address(subscribeAction);
        actionProcessDatas[0] =
            abi.encode(collectionId, address(monthlySubscribeModule), abi.encode(year, month, count));

        IDataMonetizer.ActParams memory actParams =
            IDataMonetizer.ActParams({assetId: assetId, actions: actions, actionProcessDatas: actionProcessDatas});

        vm.startPrank(actor);
        erc20Mock.approve(address(monthlySubscribeModule), amountPerMonth * count);
        dataUnion.act(actParams);
        vm.stopPrank();
    ...
    function _publish() internal returns (bytes32) {
        ...
        actionInitDatas[0] = abi.encode(address(feeCollectModule), abi.encode(totalSupply, address(erc20Mock), amount));
        actionInitDatas[1] = abi.encode(address(monthlySubscribeModule), abi.encode(address(erc20Mock), amountPerMonth));
        IDataMonetizer.PublishParams memory publishParams =
            IDataMonetizer.PublishParams({data: data, actions: actions, actionInitDatas: actionInitDatas});
    ...
    function _collect(bytes32 assetId) internal returns (uint256) {
        ...
        (uint256 collectionId,) = abi.decode(actReturnDatas[0], (uint256, bytes));
