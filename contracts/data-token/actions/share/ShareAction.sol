...
contract ShareAction is ActionBase {
    ...
    function initializeAction(
        bytes32 assetId,
        bytes calldata data
    ) external monetizerRestricted returns (bytes memory) {
        (
            string memory name,
    ...
    function processAction(
        ...
        (TradeType tradeType, uint256 amount) = abi.decode(
            data,
            (TradeType, uint256)
        );
    ...
    function _buyShare(
        ...
        uint256 ownerFeeAmount = (price * assetShareData[assetId].feePoint) /
            BASE_FEE_POINT;
        uint256 dappFeeAmount = _payDappFee(
            assetId,
            trader,
            assetShareData[assetId].currency,
            price
        );
        ...
        IERC20(assetShareData[assetId].currency).safeTransferFrom(
            trader,
            _assetOwner(assetId),
            ownerFeeAmount
        );
        IERC20(assetShareData[assetId].currency).safeTransferFrom(
            trader,
            address(this),
            price - dappFeeAmount - dataverseFeeAmount - ownerFeeAmount
        );
    ...
    function _sellShare(
        ...
        IERC20(assetShareData[assetId].currency).transfer(
            trader,
            price - dappFeeAmount - dataverseFeeAmount - ownerFeeAmount
        );
