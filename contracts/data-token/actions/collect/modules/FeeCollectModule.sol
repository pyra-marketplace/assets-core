...
contract FeeCollectModule is CollectModuleBase {
    ...
    function initializeCollectModule(
        bytes32 assetId,
        bytes calldata data
    ) external onlyCollectAction {
        (uint256 totalSupply, address currency, uint256 amount) = abi.decode(
            data,
        ...
        AssetCollectDetail memory _publicationData = AssetCollectDetail({
    ...
    function processCollect(
        ...
        address collector,
        bytes calldata data
    ) external onlyCollectAction returns (bytes memory) {
        AssetCollectDetail
            storage targetCollectDetail = _assetCollectDetailById[assetId];
        if (
            targetCollectDetail.currentCollects >=
            targetCollectDetail.totalSupply
        ) {
    ...
    function getAssetCollectDetail(
        bytes32 assetId
    ) external view returns (AssetCollectDetail memory) {
